...
## 23. 结构体复习：我的私房菜谱 (Struct & Vector)
- [ ] **作业内容**：
    1. **背景**：咱们学了那么多难的（指针、内存），这次回来复习一下最常用的——怎么存数据。就像以前做前端，我们要存一个“商品列表”或者“用户列表”。
    2. **任务**：
        - 打开 `homework_recipes.cpp`。
        - 定义一个 `Recipe` 结构体（就像 JS 的 Object）。
        - 用 `vector` 存几个菜谱。
        - 练习一下循环打印。
- [ ] **运行方法**：在终端输入 `make recipes`
- [x] **状态**：已完成

## 24. 综合实战：超市收银台 (Class & Map)
- [ ] **作业内容**：
    1. **背景**：之前的作业比较零散。这次我们模拟一个真实的场景：超市收银。
    2. **任务**：
        - 打开 `homework_supermarket.cpp`。
        - 这是一个稍微复杂点的类 `CashRegister`（收银机）。
        - 您需要用 `std::map` 来充当“价格扫码枪”数据库。
        - 还要写一个 `checkout` 方法来计算购物车总价。
    3. **知识点**：类封装、Map 查找、Vector 遍历、累加求和。
- [ ] **运行方法**：在终端输入 `make supermarket`
- [x] **状态**：已完成

## 25. 工程化起步：代码分家 (Header & Source)
- [ ] **作业内容**：
    1. **背景**：以前所有的代码都写在一个文件里，太乱了。JS 里我们用 `import/export`，C++ 里怎么做呢？
    2. **任务**：
        - `SimpleMath.h`：这里是“菜单”，告诉别人有什么功能（声明）。
        - `SimpleMath.cpp`：这里是“厨房”，真正做菜的地方（实现）。
        - `homework_split.cpp`：这里是“客人”，调用刚才写的功能（主程序）。
    3. **知识点**：头文件 (.h)、源文件 (.cpp)、编译多个文件。
- [ ] **运行方法**：在终端输入 `make split`
- [x] **状态**：已完成

## 26. 现代构建工具 (CMake)
- [x] **作业内容**：
    1. **背景**：您已经见识过 `Makefile` 的手写繁琐了。现代 C++ 项目都用 **CMake**。
    2. **改动**：
        - 我为您创建了 `CMakeLists.txt`。这就是 CMake 的“点餐单”。
        - 它会自动扫描文件，自动生成复杂的 `Makefile`。
    3. **如何使用**：
        - 以后**不需要**再手写 `make xxx` 了。
        - **编译步骤**：
          1. 创建构建目录：`mkdir build && cd build` (防止垃圾文件污染代码)
          2. 生成配置：`cmake ..` (告诉 cmake 源代码在上一级目录)
          3. 开始编译：`make` (这时候生成的 Makefile 就在 build 目录里)
        - **运行程序**：编译好的程序都在 `build` 目录里，比如 `./split_test`。
- [x] **状态**：已配置完成
